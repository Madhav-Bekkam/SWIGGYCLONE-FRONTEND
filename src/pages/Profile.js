...
export default function Profile() {
  ...
  const fetchBusinessSettings = async () => {
    try {
      const res = await axios.get("https://swiggyclone-backend-1.onrender.com/api/settings");
  ...
  const handleUpdateBusiness = async (e) => {
    e.preventDefault();
    try {
      await axios.put("https://swiggyclone-backend-1.onrender.com/api/settings", businessInfo, config);
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.put("https://swiggyclone-backend-1.onrender.com/api/users/profile", profileData, config);
